# Hi, I'm Zhangrui Yang 👋
<p align="left">
  <a href="https://YARATHUSTRA.github.io">
    <img src="https://img.shields.io/badge/Personal%20Website-Visit%20Now-blue?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Website">
  </a>
  <a href="https://scholar.google.com/citations?user=1io0foEAAAAJ">
    <img src="https://img.shields.io/badge/Google%20Scholar-Profile-red?style=for-the-badge&logo=googlescholar&logoColor=white" alt="Scholar">
  </a>
</p>
Undergraduate in Computer Science at Tongji University (Elite Program)

I am interested in **Vision-Language Models, Spatial Reasoning, and Multimodal Learning**.  
My research focuses on understanding how large multimodal models acquire structured spatial knowledge and how such reasoning capabilities can be evaluated and improved.

---

## 🔬 Research Interests

- Vision-Language Models (VLM)
- Spatial Reasoning
- Multimodal Learning
- Robust Visual Perception
- Machine Learning Systems

---

## 📄 Publications

**Pseudo-Label Refinement for Robust Wheat Head Segmentation via Two-Stage Hybrid Training**  
*Co-first Author*  
Accepted at **CVPPA Workshop @ ICCV 2025**
https://arxiv.org/abs/2512.11874

Authors: Jiahao Jiang, Zhangrui Yang, Xuanhan Wang, Jingkuan Song

---

## 🏆 Selected Achievements

- 🥈 **Global Wheat Full Semantic Segmentation (GWFSS) Competition – Global 2nd Place**
- 🥇 **iCAN National Innovation Competition – First Prize**
- 📊 **National Mathematical Modeling Contest – Shanghai Second Prize**

---

## 🧠 Current Focus

Currently exploring:

- Spatial reasoning capabilities in large vision-language models
- Benchmark design for multimodal spatial understanding
- Reproduction and analysis of recent CVPR / ICCV / NeurIPS papers

---

## 💻 Technical Skills

**Languages**

- Python
- C++
- Verilog HDL
- SQL

**Machine Learning**

- PyTorch
- Computer Vision
- Data Processing & Experiment Pipelines


---

## 🌐 Academic Profiles

- Personal website: https://YARATHUSTRA.github.io
- Email: jerry.zryang@hotmail.com
- Google Scholar: https://scholar.google.com/citations?user=1io0foEAAAAJ&hl=zh-CN
